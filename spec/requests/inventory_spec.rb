...
RSpec.describe "Inventory", type: :request do
  ...
  describe "GET /inventory/:type_slug (type_show)" do
    it "returns a successful response" do
      get inventory_type_path(plant_type.slug)
      expect(response).to have_http_status(:ok)
    end
  ...
  describe "GET /inventory/:type_slug/:category_slug (category_show)" do
    it "returns a successful response" do
      get inventory_category_path(plant_type.slug, plant_category.slug)
      expect(response).to have_http_status(:ok)
    end
    ...
    it "shows subcategory links when present" do
      get inventory_category_path(plant_type.slug, plant_category.slug)
      expect(response.body).to include(plant_subcategory.name)
    end

    it "shows Research Growing Guide button" do
      get inventory_category_path(plant_type.slug, plant_category.slug)
      expect(response.body).to include("Research Growing Guide")
    end
  end

  describe "GET /inventory/:type_slug/:category_slug/:subcategory_slug (subcategory_show)" do
    it "returns a successful response" do
      get inventory_subcategory_path(plant_type.slug, plant_category.slug, plant_subcategory.slug)
      expect(response).to have_http_status(:ok)
    end
  ...
  describe "GET /inventory/:type_slug/:category_slug/variety/:plant_slug (variety_show)" do
    it "returns a successful response" do
      get inventory_variety_path(plant_type.slug, plant_category.slug, heirloom_plant.slug)
      expect(response).to have_http_status(:ok)
    end

    it "shows plant detail" do
      get inventory_variety_path(plant_type.slug, plant_category.slug, heirloom_plant.slug)
      expect(response.body).to include("Brandywine")
      expect(response.body).to include("Plant Details")
    end

    it "shows breadcrumb with plant name" do
      get inventory_variety_path(plant_type.slug, plant_category.slug, heirloom_plant.slug)
      expect(response.body).to include("Brandywine")
    end
  end

  describe "GET /inventory/:type_slug/:category_slug/:subcategory_slug/variety/:plant_slug (subcategory_variety_show)" do
    it "returns a successful response" do
  ...
  describe "GET /inventory/browse (browse_redirect)" do
    it "redirects to type page when plant_type_id given" do
      get inventory_browse_path(plant_type_id: plant_type.id)
      expect(response).to have_http_status(:moved_permanently)
      expect(response).to redirect_to(inventory_type_path(plant_type.slug))
    end

    it "redirects to category page when plant_category_id given" do
      get inventory_browse_path(plant_category_id: plant_category.id)
      expect(response).to have_http_status(:moved_permanently)
      expect(response).to redirect_to(inventory_category_path(plant_type.slug, plant_category.slug))
    end

    it "redirects to subcategory page when plant_subcategory_id given" do
      get inventory_browse_path(plant_subcategory_id: plant_subcategory.id)
      expect(response).to have_http_status(:moved_permanently)
      expect(response).to redirect_to(inventory_subcategory_path(plant_type.slug, plant_category.slug, plant_subcategory.slug))
    end

    it "redirects to root when no params given" do
  ...
  describe "POST /inventory/research_growing_guide" do
    it "enqueues a GrowingGuideResearchJob for a category" do
      expect {
        post inventory_research_growing_guide_path, params: { guideable_type: "PlantCategory", guideable_id: plant_category.id },
             headers: { "HTTP_REFERER" => inventory_category_path(plant_type.slug, plant_category.slug) }
      }.to have_enqueued_job(GrowingGuideResearchJob).with(plant_category.id, "PlantCategory")
    end

    it "enqueues a GrowingGuideResearchJob for a subcategory" do
      expect {
        post inventory_research_growing_guide_path, params: { guideable_type: "PlantSubcategory", guideable_id: plant_subcategory.id },
             headers: { "HTTP_REFERER" => inventory_subcategory_path(plant_type.slug, plant_category.slug, plant_subcategory.slug) }
      }.to have_enqueued_job(GrowingGuideResearchJob).with(plant_subcategory.id, "PlantSubcategory")
    end

    it "redirects back with notice" do
      post inventory_research_growing_guide_path,
           params: { guideable_type: "PlantCategory", guideable_id: plant_category.id },
           headers: { "HTTP_REFERER" => inventory_category_path(plant_type.slug, plant_category.slug) }
      expect(response).to redirect_to(inventory_category_path(plant_type.slug, plant_category.slug))
  ...
  describe "plant list content" do
  ...
  describe "filter functionality" do
    ...
    it "renders filter buttons on type page" do
      get inventory_type_path(plant_type.slug)
      expect(response.body).to include("Viable Only")
    end
    ...
    context "combining filters" do
      it "combines viability and taxonomy navigation" do
        get inventory_type_path(plant_type.slug), params: { viability: "viable" }
        expect(response).to have_http_status(:ok)
        expect(response.body).to include("Sun Gold")
        expect(response.body).not_to include("Brandywine")
      end
  ...
  describe "bulk select UI" do
    ...
    it "shows bulk select checkboxes on type page" do
      get inventory_type_path(plant_type.slug)
      expect(response.body).to include("bulk-select")
      expect(response.body).to include("inventory-bulk-form")
    end
  ...
  describe "search functionality" do
    ...
    it "displays a search bar on the type page" do
      get inventory_type_path(plant_type.slug)
      expect(response.body).to include("Search plants")
    end
    ...
    context "search combined with taxonomy navigation" do
      ...
      it "searches within a specific plant type" do
        get inventory_type_path(plant_type.slug), params: { q: "Sun" }
        expect(response).to have_http_status(:ok)
        expect(response.body).to include("Sun Gold")
      ...
      it "searches within a specific category" do
        get inventory_category_path(plant_type.slug, plant_category.slug), params: { q: "Brandywine" }
        expect(response).to have_http_status(:ok)
        expect(response.body).to include("Brandywine")
      end
    end
